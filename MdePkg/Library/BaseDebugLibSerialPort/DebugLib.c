...
#define ISO_CHAR_HEIGHT 8

unsigned long VideoAddr = 0x20028000;
unsigned long TextCursorX = 0;
unsigned long TextCursorY = 0;
unsigned long FbWidth = 1280;
unsigned long FbHeight = 720;
unsigned long FbPPSL = 1280;
unsigned long TextConsoleWidth = 160;
unsigned long TextConsoleHeight = 90;


void bootVideoConsolePrintChar(char8 c)
{
    uintn   delta;
    uint32  top;

    if (video->textCursorX >= video->textConsoleWidth)
    {
        video->textCursorX = 0;
        video->textCursorY++;
    }

    unsigned long needsScroll   = (TextCursorY >= video->textConsoleHeight);
    if (video->needsScroll)
    {
        delta = (video->fbPitch + 3) & ~0x3;
        top = (delta * ISO_CHAR_HEIGHT);

        memmove((void *) video->fbBase, (const void *) VideoAddr + top, video->fbSize - top);
        memset((void *) video->fbBase + video->fbSize - top, 0, top);
        video->textCursorY -= 1;
    }
    ...
    switch (c)
    {
        ...
        {
            video->textCursorX--;
            bootVideoPrintChar(c,
                               video->textCursorX,
                               video->textCursorY,
                               video->textForegroundColor,
                               video->textBackgroundColor);
        }
