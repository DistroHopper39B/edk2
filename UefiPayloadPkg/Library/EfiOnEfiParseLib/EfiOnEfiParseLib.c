/** @file
  This library will parse the EFI-On-EFI information struct and extract those required
  information.

  Copyright (c) 2026, Sylas Hollander. All rights reserved.<BR>
...
{
  EFI_ON_EFI_INFO  *HandoffData;
  //
  // Efi-On-Efi will pass the information structure to BootloaderParameter
  //
  
  HandoffData = (EFI_ON_EFI_INFO *) (UINTN) GET_BOOTLOADER_PARAMETER ();
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "HandoffData->MemoryMap->Count = %d\n", HandoffData->MemoryMap->Count));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "Video height: %d\n", HandoffData->FbHeight));
  DEBUG ((DEBUG_INFO, "Video pitch: %d\n", HandoffData->FbBytesPerRow));
  
  GfxMode                                 = &GfxInfo->GraphicsMode;
  
  GfxMode->Version                        = 0;
  GfxMode->HorizontalResolution           = HandoffData->FbWidth;
  GfxMode->VerticalResolution             = HandoffData->FbHeight;
  GfxMode->PixelsPerScanLine              = HandoffData->FbBytesPerRow / 4;
  ...
  GfxInfo->FrameBufferBase = (EFI_PHYSICAL_ADDRESS)(UINTN)HandoffData->FbBase;
  GfxInfo->FrameBufferSize = HandoffData->FbBytesPerRow * HandoffData->FbHeight;
